...
import { Heart, ChevronRight, Expand } from "lucide-react";
...
export interface Product {
  ...
  name: string;
  price: string;
  /** Alias of price for display contexts. */
  priceText?: string;
  tag: string;
  sizes?: string[];
  colors?: ProductColor[];
...
export function ProductGrid({ products }: { products: Product[] }) {
  ...
  const openProduct = (p: Product) => {
    setActive({ ...p, priceText: p.priceText ?? p.price });
    setOpen(true);
  };
  ...
  return (
    <>
          ...
          <Reveal key={i} delay={i * 90} variant="tilt" y={40}>
            <Tilt3D max={16} lift={18} glare={0.4} scale={1.05} className="rounded-[24px]">
              <article
                ...
                className="group relative aspect-[3/4] cursor-pointer overflow-hidden rounded-[24px] bg-muted shadow-[0_20px_50px_-20px_rgba(0,0,0,0.35)]"
              >
                <div
                  data-tilt-layer="14"
                ...
                >
                  <span className="flex items-center gap-2 rounded-pill bg-background/90 px-4 py-2 text-[10px] font-semibold tracking-[0.18em] backdrop-blur">
                    <Expand className="size-3.5" /> QUICK VIEW
                  </span>
                </div>
      ...
      <ProductQuickView product={active} open={open} onOpenChange={setOpen} />
    </>
  );
}
